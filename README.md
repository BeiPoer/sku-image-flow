...
- **普通模板**：一套详情图节点流程（节点名称、顺序、比例、提示词、是否依赖主图都可自定义）
- **镜像模板**：上传多张参考图，每张参考图自动成为一个镜像节点，用于按相同构图/场景/风格复刻新商品图
...
1. 新建镜像模板并上传多张参考图（一张图就是一个镜像节点）
2. 在模板下新建 SKU，上传 1 张商品图
3. 可选填写 SKU 全局提示词
4. 一键生成每个镜像节点的复刻候选图
5. 每个节点人工选择最终图并导出 zip
...
镜像模板不手动配置节点提示词。创建模板时上传参考图，系统会按上传顺序把每张图保存为一个镜像节点：

- 每张参考图负责一个复刻结果
- 可在模板设置里追加、排序、删除参考图
- 删除参考图采用软删除：历史候选图仍可保留和导出
- 镜像 SKU 只保留 1 张商品图；再次上传会替换旧商品图
- SKU 全局提示词会参与该 SKU 下所有镜像节点生成
- 每个镜像节点仍支持「重跑修正」和临时参考图，仅本次生成生效
